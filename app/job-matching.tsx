...
export function JobMatching({ notify, openStudio, applicationView }: { notify: (message: string, tone?: 'success' | 'error') => void; openStudio: (step: number, id?: string) => void; applicationView?: ReactNode }) {
  ...
  return <section className="space-y-4">
    ...
    {loading ? <p role="status">Loading saved jobs…</p> : !loaded ? <Button variant="outline" onClick={() => setRetry(x => x + 1)}>Retry loading jobs</Button> : tab === 'Catalog' ? <>
      ...
      <JobsWorkspace jobs={data.jobs} matches={data.matches} state={state} review={m => { setSelected(m); setModal('review'); }} edit={j => { setEditJob(j); setSuggestion(null); setSuggestionApplied(false); setModal('job'); }} openStudio={openStudio} updateStatus={(id, status) => { void perform(() => act('job.status', { id, status }), 'Application status updated.'); }} />
    </> : tab === 'Matches' ? <>
      ...
      </div><p className="my-4 text-sm text-slate-600">Empty eligibility fields require admin review. Skills entered by candidates or admins count without additional evidence notes. Secondary families must exist in Intelligence.</p><Button type="submit" disabled={pending || !candidate}>Save preferences</Button>
    </form>}
    <Dialog open={modal !== null} onOpenChange={open => { if (!open && !pending) { setModal(null); setSuggestion(null); setSuggestionApplied(false); } }}><DialogContent className="max-h-[90vh] overflow-y-auto rounded-md sm:max-w-5xl"><DialogHeader><DialogTitle>{modal === 'job' ? editJob ? 'Edit shared job' : 'Add shared job' : modal === 'import' ? 'Import job catalog' : 'Candidate qualifications and decision'}</DialogTitle><DialogDescription>{modal === 'job' ? 'One JD can match multiple candidates. Required fields must reflect the actual job posting.' : modal === 'import' ? 'Upload up to 100 jobs. Invalid rows are reported before import starts.' : 'Review candidate qualifications, eligibility and score components.'}</DialogDescription></DialogHeader>
      ...
      }} />}<form className="space-y-4" onSubmit={e => { e.preventDefault(); const form = new FormData(e.currentTarget); void perform(async () => { await decideMatch(user, selected.id, 'Approved', String(form.get('reason') ?? '')); setModal(null); }, 'Match approved. Application is ready in Resume studio.'); }}><div className="grid grid-cols-2 gap-3 sm:grid-cols-5">{Object.entries(selected.scores).map(([name, value]) => <div key={name} className="rounded border p-3 text-sm"><p>{name}</p><strong>{value}%</strong></div>)}</div><p>Eligibility: <strong>{selected.eligibility}</strong> · Match: <strong>{selected.score}%</strong></p><ul className="list-disc space-y-1 pl-5 text-sm">{selected.reasons.map(r => <li key={r}>{r}</li>)}</ul><p className="text-sm">JD skills not listed in candidate profile: {selected.missingMandatory.join(', ') || 'None'}</p>{selected.applicationId ? <Button type="button" onClick={() => { setModal(null); openStudio(3, selected.applicationId); }}>Open in Resume studio</Button> : <><label className="grid gap-1 text-sm">Review reason<textarea name="reason" className={fieldClass} rows={3} /></label><div className="flex gap-2"><Button type="submit" disabled={pending || selected.eligibility === 'Ineligible' || selected.score < 70}>Approve match</Button><Button type="button" variant="outline" disabled={pending} onClick={e => { const reason = String(new FormData(e.currentTarget.form!).get('reason') ?? ''); void perform(async () => { await decideMatch(user, selected.id, 'Rejected', reason); setModal(null); }, 'Review decision saved.'); }}>Reject match</Button></div></>}</form></> : null}
