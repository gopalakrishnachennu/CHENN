...
import { QualificationReview } from '@/components/resumes/qualification-review';
...
function suggestionFor(input: { title: string; company?: string; family?: string; jdText: string; workType?: string; intelligence?: CatalogJob['intelligence']; requirements?: CatalogJob['requirements'] }, families: JobFamily[]): JobSuggestion {
  ...
  const attention = [!family.family && 'Confirm job family', family.confidence < 80 && 'Review low-confidence family', !mandatorySkills.length && 'Confirm mandatory skills', !structured.authorization && 'Confirm work authorization'].filter(Boolean) as string[];
...
export function JobMatching({ notify, openStudio, applicationView }: { notify: (message: string, tone?: 'success' | 'error') => void; openStudio: (step: number, id?: string) => void; applicationView?: ReactNode }) {
  ...
  const submitJob = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault(); if (!suggestionApplied) { setError('Analyze and accept the current JD before saving.'); return; } const values = Object.fromEntries(new FormData(event.currentTarget));
    await perform(async () => {
  ...
  return <section className="space-y-4">
    ...
    {loading ? <p role="status">Loading saved jobs…</p> : !loaded ? <Button variant="outline" onClick={() => setRetry(x => x + 1)}>Retry loading jobs</Button> : tab === 'Catalog' ? <>
    ...
    </> : tab === 'Applications' ? applicationView : <form key={candidateId + candidate?.updatedAt} className="rounded border bg-white p-4" onSubmit={e => { e.preventDefault(); const values = Object.fromEntries(new FormData(e.currentTarget)); void perform(() => savePreferences(user, candidateId, values as never), 'Preferences saved and matches recalculated.'); }}>
      ...
      </div><p className="my-4 text-sm text-slate-600">Empty eligibility fields require admin review. Skills count only when the candidate profile contains supporting evidence. Secondary families must exist in Intelligence.</p><Button type="submit" disabled={pending || !candidate}>Save preferences</Button>
    </form>}
    <Dialog open={modal !== null} onOpenChange={open => { if (!open && !pending) { setModal(null); setSuggestion(null); setSuggestionApplied(false); } }}><DialogContent className="max-h-[90vh] overflow-y-auto rounded-md sm:max-w-5xl"><DialogHeader><DialogTitle>{modal === 'job' ? editJob ? 'Edit shared job' : 'Add shared job' : modal === 'import' ? 'Import job catalog' : 'Match evidence and decision'}</DialogTitle><DialogDescription>{modal === 'job' ? 'One JD can match multiple candidates. Required fields must reflect the actual job posting.' : modal === 'import' ? 'Upload up to 100 jobs. Invalid rows are reported before import starts.' : 'Check eligibility, missing evidence and score components before continuing.'}</DialogDescription></DialogHeader>
      ...
      {modal === 'job' ? <form ref={jobForm} onChange={e => { if (['title', 'jdText', 'company'].includes((e.target as unknown as HTMLInputElement).name)) { setSuggestion(null); setSuggestionApplied(false); } }} onSubmit={submitJob} className="space-y-4"><div className="flex flex-wrap items-center justify-between gap-3 rounded border border-indigo-200 bg-indigo-50 p-3"><div><p className="text-sm font-semibold text-indigo-950">Let the analyzer prepare the review</p><p className="text-xs text-indigo-700">Six required fields. Salary must reflect the posting (or explicitly state Not disclosed). AI analyzes once when your key is connected; otherwise rule-based suggestions are shown.</p></div><Button type="button" variant="outline" disabled={pending} onClick={() => void prepareSuggestion(editJob)}><Sparkles className="size-4" /> Analyze JD</Button></div><div className="grid gap-3 sm:grid-cols-2">
      ...
      <details className="rounded border bg-slate-50"><summary className="cursor-pointer px-4 py-3 text-sm font-semibold">Analyzed fields and optional corrections</summary><div className="grid gap-3 border-t p-4 sm:grid-cols-2 lg:grid-cols-3">
