...
export function CareerEditor({ value, onChange }: { value: Career; onChange: (v: Career) => void }) {
  ...
  return <section className="wide-row col-span-full space-y-3 rounded border p-4">
    ...
    {rows.map((row, i) => <fieldset key={`${tab}-${i}`} className="grid gap-3 rounded border p-3 sm:grid-cols-2 lg:grid-cols-3"><legend className="px-2 text-sm capitalize">{tab} {i + 1}</legend>{Object.entries(definitions[tab]).map(([key, label]) => <label key={key} className="grid gap-1 text-sm">{label}{key === 'current' ? <input type="checkbox" checked={Boolean(row[key])} onChange={e => update(rows.map((r, n) => n === i ? { ...r, current: e.target.checked, ...(e.target.checked ? { end: '' } : {}) } : r))} /> : <input className="min-w-0 rounded border px-3 py-2" type={key === 'start' || key === 'end' ? 'month' : key === 'url' ? 'url' : 'text'} disabled={key === 'end' && Boolean(row.current)} value={String(row[key] ?? '')} onChange={e => update(rows.map((r, n) => n === i ? { ...r, [key]: e.target.value } : r))} />}</label>)}<button type="button" className="text-left text-sm text-red-700" onClick={() => update(rows.filter((_, n) => n !== i))}>Remove entry</button></fieldset>)}
