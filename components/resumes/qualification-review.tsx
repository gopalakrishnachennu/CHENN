...
export function QualificationReview({ candidate, job, save, busy }: { candidate: Candidate; job: Pick<Job, 'jdProfile' | 'mandatorySkills' | 'preferredSkills'>; save: (items: Item[]) => Promise<void>; busy: boolean }) {
  ...
  const suggestions: Array<Pick<Item, 'kind' | 'name'>> = [
    ...[...new Set([...(job.jdProfile?.mandatory_skills ?? job.mandatorySkills), ...(job.jdProfile?.required_skills ?? []), ...(job.jdProfile?.preferred_skills ?? job.preferredSkills)])].filter(name => !candidate.skills.some(s => s.name.toLowerCase() === name.toLowerCase() && ['Profile', 'Career'].includes(s.source))).map(name => ({ kind: 'skill' as const, name })),
