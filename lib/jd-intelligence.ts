...
export function analyzeJD(input: { title: string; jdText: string; company?: string; family?: string }, candidate?: Candidate, knowledge: JobFamilyKnowledge[] = JOB_FAMILY_KNOWLEDGE): JDAnalysis {
  ...
  const keywords = requirements.filter(item => ['skill', 'platform', 'tool', 'methodology', 'responsibility'].includes(item.kind)); const domain = detectDomain(jd);
